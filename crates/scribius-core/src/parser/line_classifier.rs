...
mod tests {
    ...
    fn test_login() {
        let db = test_db();
        let event = classify_line("Welcome to Clan Lord, Gandor!", &db);
        assert!(matches!(event, LogEvent::Login { ref name } if name == "Gandor"));
    }
    ...
    fn test_yen_healing_sense_ignored() {
        let db = test_db();
        let event = classify_line("¥You sense healing energy from Gandor.", &db);
    ...
    fn test_speech_ignored() {
        ...
        assert!(matches!(
            classify_line(r#"Gandor says, "hello""#, &db),
            LogEvent::Ignored
        ));
    }
    ...
    fn test_loot_share() {
        let db = test_db();
        let event = classify_line(
            "* Gandor recovers the Dark Vermine fur, worth 20c. Your share is 10c.",
            &db,
        );
        assert!(matches!(
            event,
    ...
    fn test_fallen() {
        let db = test_db();
        let event = classify_line("Gandor has fallen to a Large Vermine.", &db);
        assert!(matches!(
            event,
            LogEvent::Fallen {
                ref name,
                ref cause
            } if name == "Gandor" && cause == "Large Vermine"
        ));
    }
    ...
    fn test_chain_used() {
        let db = test_db();
        let event = classify_line("You start dragging Helga.", &db);
        assert!(matches!(
            event,
            LogEvent::ChainUsed { ref target } if target == "Helga"
        ));
    }
    ...
    fn test_karma_good() {
        let db = test_db();
        let event = classify_line("You just received good karma from Gandor.", &db);
