...
mod tests {
    ...
    fn test_welcome_login() {
        let caps = WELCOME_LOGIN.captures("Welcome to Clan Lord, Gandor!").unwrap();
    ...
    #[test]
    fn test_fallen() {
        let caps = FALLEN.captures("Gandor has fallen to a Large Vermine.").unwrap();
        assert_eq!(&caps[1], "Gandor");
    ...
    #[test]
    fn test_fallen_acid() {
        let caps = FALLEN.captures("Gandor has fallen to a spray of acid.").unwrap();
    ...
    fn test_loot_share_fur() {
        let caps = LOOT_SHARE.captures("* Gandor recovers the Dark Vermine fur, worth 20c. Your share is 10c.").unwrap();
    ...
    #[test]
    fn test_chain_drag() {
        let caps = CHAIN_DRAG.captures("You start dragging Helga.").unwrap();
        assert_eq!(&caps[1], "Helga");
    }

    #[test]
    fn test_speech_skip() {
        assert!(SPEECH.is_match(r#"Donk thinks, "south""#));
        assert!(SPEECH.is_match(r#"Gandor says, "hello""#));
        assert!(SPEECH.is_match(r#"Gandor yells, "help!""#));
    }

    #[test]
    fn test_emote_skip() {
        assert!(EMOTE.is_match("(Gandor waves)"));
    }
    ...
    #[test]
    fn test_karma_good() {
        let caps = KARMA_RECEIVED.captures("You just received good karma from Gandor.").unwrap();
    ...
    fn test_karma_without_just() {
        let caps = KARMA_RECEIVED.captures("You received good karma from Gandor.").unwrap();
        assert_eq!(&caps[1], "good");
    }
