...
1/1/24 1:01:00p Gandor says, "hello everyone"
1/1/24 1:02:00p (Gandor waves)
...
";
    ...
    fn test_loot_share_tracking() {
        ...
        let log_content = "\
1/1/24 1:00:00p * Gandor recovers the Dark Vermine fur, worth 20c. Your share is 10c.
...
";
    ...
    fn test_extract_character_name_login() {
        let bytes = b"1/1/24 1:00:00p Welcome to Clan Lord, Gandor!\n";
        assert_eq!(extract_character_name(bytes), Some("Gandor".to_string()));
    }
    ...
    fn test_titlecase_name_single_word() {
        assert_eq!(titlecase_name("ruuk"), "Gandor");
        assert_eq!(titlecase_name("RUUK"), "Gandor");
        assert_eq!(titlecase_name("Gandor"), "Gandor");
    }
...
";
    ...
    fn test_highest_kill_query() {
        let db = Database::open_in_memory().unwrap();
        let id = db.get_or_create_character("Gandor").unwrap();
