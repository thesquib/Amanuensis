...
9. **Kill frequency**: the `kill_events` table records one timestamped row per kill (creature, verb, assisted flag) during scan — the per-event basis for windowed stats (the aggregated `kills` table is unchanged). Per creature, two max-ever metrics are derived in `crates/amanuensis-core/src/db/queries/frequency.rs` (the single source of truth shared by GUI and CLI): **best calendar day** (highest kills in any fixed midnight-aligned 24h day, via SQL bucket) and **best 2 hours** (highest kills in any 2h *sliding* window, via a two-pointer sweep — catches bursts that fixed bins would split). Surfaced as **Best Day / Best 2h** columns in KillsView (Tauri `get_kill_frequency`) and via the CLI `amanuensis frequency <char> [--bin day|2h|both] [--solo] [--by-verb] [--format table|csv|json] [--limit N]`. Both surfaces include assisted kills by default (`--solo` for solo-only). Origin: requested by the upstream bestiary-data author for quantitative spawn-frequency collation. **Databases scanned before this feature need a full re-scan to backfill `kill_events`.** Use the GUI's **Rescan Logs** action (`rescan_sources`, which calls `reset_log_data` first and re-scans all sources) — this is the safe path and repopulates `kill_events` without double-counting. For the CLI, use `amanuensis rescan <folder...>` (resets derived data via `reset_log_data`, then re-scans — safe, no double-count; pass ALL your log folders since the reset wipes derived data first). Do **not** use `amanuensis scan --force <folder>` for backfill: `--force` only bypasses the already-scanned skip-guards and does not reset first, so re-scanning an already-scanned folder double-counts both `kill_events` and the aggregated `kills` totals (a pre-existing `--force` behavior).
